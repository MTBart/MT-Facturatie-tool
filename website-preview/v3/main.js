/* =========================================================================
   MORTISE & TENON — prototype 3D-website  (v3)
   -------------------------------------------------------------------------
   v3-regie (GESPIEGELD, per Barts aanwijzing 3-7):
     - De deurhoek staat vanaf het begin FACE-ON in beeld: stijl LINKS
       (verticaal), de regel schuift VAN RECHTS naar binnen. Niets draait —
       geen camera-zwenk, geen werkstuk-rotatie.
     - Echte deur-maten: stijl/regel ±65mm breed, ±21mm dik (1 eenheid ~
       100mm). HAUNCHED tenon in een bovenhoek, paneelgroef + paneel.
     - De dwarsdoorsnede (clip halverwege de dikte) is LETTERLIJK het
       M&T-logo: alle maten zijn uit de logo-SVG afgeleid (kader 97.8,
       deellijn 55.5 -> 33.9 tussen y 30.9..67.6).
     - Volgorde: aanschuiven tot vlak voor het gat -> doorsnede opent ->
       de pen valt IN doorsnedebeeld in het gat ("valt over elkaar heen")
       -> tekenlaag trekt kader + naadlijn -> naadlijn morpht naar de
       logo-deellijn -> crossfade naar het echte merk (pixel-uitgelijnd)
       -> iris-reveal.
...
var Intro = (function () {
  ...
  /* Tijdlijn in seconden — totaal ca. 13s
  ...
  /* ---------------- procedurele meubel-eiken-textuur ---------------- */
  function woodTexture(cfg){
    var W = 1024, H = 1024, i, x, y;
    ...
    grad.addColorStop(0.45, cfg.base[1]);
    grad.addColorStop(1,    cfg.base[2]);
    ctx.fillStyle = grad;
    ctx.fillRect(0, 0, W, H);

    /* fijne, dicht op elkaar liggende nerven — meubelhout, niet ruwe balk */
    for (i = 0; i < 230; i++){
      var y0 = Math.random() * H;
      var amp = rand(1.2, 8);
      var wl  = rand(220, 560);
      var ph  = Math.random() * Math.PI * 2;
      var donker = Math.random() < 0.8;
      ctx.strokeStyle = donker
        ? 'rgba(66,42,22,'    + rand(0.04, 0.11).toFixed(3) + ')'
        : 'rgba(236,206,158,' + rand(0.02, 0.06).toFixed(3) + ')';
      ctx.lineWidth = rand(0.5, 2.0);
      ctx.beginPath();
      for (x = 0; x <= W; x += 12){
        y = y0 + Math.sin(x / wl * Math.PI * 2 + ph) * amp
               + Math.sin(x / 37 + ph * 3) * 1.0;
        if (x === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y);
      }
      ctx.stroke();
    }

    /* enkele zachte vroeg-/laathout-banen */
    for (i = 0; i < 7; i++){
      ctx.fillStyle = 'rgba(74,48,24,' + rand(0.02, 0.05).toFixed(3) + ')';
      ctx.fillRect(0, Math.random() * H, W, rand(14, 44));
    }

    /* enkele fijne kwast — meubelhout is rustiger */
    for (i = 0; i < cfg.knots; i++){
      var kx = rand(90, W - 90), ky = rand(70, H - 70), kr = rand(6, 13);
      var kg = ctx.createRadialGradient(kx, ky, 1, kx, ky, kr * 2.4);
      kg.addColorStop(0,    'rgba(52,32,14,0.55)');
      kg.addColorStop(0.4,  'rgba(74,46,20,0.22)');
      kg.addColorStop(1,    'rgba(74,46,20,0)');
      ctx.fillStyle = kg;
      ctx.beginPath();
      ctx.ellipse(kx, ky, kr * 2.4, kr * 1.3, 0, 0, Math.PI * 2);
      ctx.fill();
    }
    ...
    for (i = 0; i < 3000; i++){
      ctx.fillStyle = (Math.random() < 0.5)
        ? 'rgba(255,230,188,0.02)'
        : 'rgba(34,20,10,0.04)';
      ctx.fillRect(Math.random() * W, Math.random() * H, rand(1, 2), rand(1, 1.6));
    }
  ...
  /* ---------------- doorsnede-vlakken (caps) ----------------
     ...
     zijn de "zaagsnede" — het kopshout. De stijl-cap heeft het gat en de
     groeven als echte uitsparing; de regel-cap (pen + haunch-profiel)
  ...
  function buildCaps(){
    ...
    var matS = capMat(0x8a6a45);   // stijl — kopshout, toon A
    var matR = capMat(0x97764f);   // regel — toon B (bewust nét anders)
    var matP = capMat(0xa2825c);   // paneel
    ...
    railGroup.add(shapeMesh(panelPts, matP, 0.006));
  ...
  function buildTrace(){
    ...
    function strokePartial(pts, frac){
      ...
      for (i = 0; i < segs.length; i++){
        if (run + segs[i] <= want){
          ...
          p = toPx([ pts[i][0] + (pts[i + 1][0] - pts[i][0]) * k,
    ...
    function draw(sqF, bdF, m, bgA){
      ...
      ctx.strokeStyle = 'rgba(236,229,224,0.97)';
      ctx.lineWidth = LW;
      ctx.lineCap = 'round';
      ctx.lineJoin = 'round';
      strokePartial(SQ, sqF);
      if (bdF > 0){
        var pts = [], i;
  ...
  function makeDust(){
    ...
    for (i = 0; i < MAX; i++){
      ...
      col[i3] = 0.55 * w; col[i3 + 1] = 0.46 * w; col[i3 + 2] = 0.30 * w;
      motes.push({
        bx: mx, by: my, bz: mz,
    ...
    function updateWisp(dt){
      for (var k = 0; k < wisps.length; k++){
        ...
        col[i3] = 0.6 * a; col[i3 + 1] = 0.5 * a; col[i3 + 2] = 0.32 * a;
      }
    }
  ...
  function buildScene(){
    ...
    /* mildere fog dan v2: de eindcamera staat ver weg (tele) en mag de
    ...
    /* licht: één warme bovenlichtbundel van schuin-voor; rest duister */
    var amb = new THREE.AmbientLight(0x2a2022, 0.42);
    scene.add(amb);

    spot = new THREE.SpotLight(0xffd7a0, 2.2, 0, 0.42, 0.72, 1.0);
    spot.position.set(-1.6, 4.6, 3.2);
    spot.target.position.set(0.2, -0.2, 0);
    ...
    var rim = new THREE.DirectionalLight(0xa89a90, 0.22);
    rim.position.set(2.5, 1.0, -2.5);
    scene.add(rim);

    /* materialen — satijn-eiken */
    var texA = woodTexture({ base: ['#8f6740', '#a67c4e', '#916b40'], knots: 2 });
    var texB = woodTexture({ base: ['#997049', '#ac8156', '#946c40'], knots: 1 });
    var texP = woodTexture({ base: ['#a9814f', '#b98f5e', '#9c7548'], knots: 0 });

    var matA = new THREE.MeshStandardMaterial({
      map: texA, roughness: 0.52, metalness: 0.0, bumpMap: texA, bumpScale: 0.008
    });
    var matB = new THREE.MeshStandardMaterial({
      map: texB, roughness: 0.5, metalness: 0.0, bumpMap: texB, bumpScale: 0.008
    });
    var matPanel = new THREE.MeshStandardMaterial({
      map: texP, roughness: 0.58, metalness: 0.0, bumpMap: texP, bumpScale: 0.006
    });
    var matDark = new THREE.MeshBasicMaterial({
      color: 0x080605, side: THREE.DoubleSide, fog: false
    });
    ...
    jointMats = [matA, matB, matPanel, matDark];
    ...
    /* een paar vage werkbank-silhouetten in de diepte */
    var silMat = new THREE.MeshStandardMaterial({ color: 0x120d0b, roughness: 0.98 });
    var silhouettes = [
    ...
    silhouettes.forEach(function (s){
      var m = new THREE.Mesh(new THREE.BoxGeometry(s.w, s.h, s.d), silMat);
    ...
    railGroup.add(buildPanel(matPanel));
